...
from models.lightgcn.modules.dataset import LightGCNData, LightGCNTest, NegativeSampler
from models.lightgcn.modules.model import LightGCN
from models.lightgcn.utils.helper import get_filtered_dataset_from_s3
...
class LightGCNTrainer:
    def __init__(self, **config):
        ...
        self.save_mapping()
        train_ds = LightGCNData(self.data_dict)
        test_ds = LightGCNTest(self.data_dict)
        ...
        self.model = LightGCN(
            ...
            graph_matrix=[device_product_norm, product_device_norm],
        ).to(self.device)
    ...
    def save_checkpoint(self, e):
        ...
        client.put_object(
            Body=open("/tmp/lightgcn.pth", "rb"),
            Bucket=self.config["model_s3_bucket_name"],
            Key="{}/{}/{}/{}/lightgcn_{}.pth".format(
                self.config["company_id"],
                "lightgcn",
                self.config["etl_id"],
                self.config["model_id"],
    ...
    def train(self):
        ...
        for e in range(start_epoch, start_epoch + self.config["train_epoch"]):
            ...
            self.record_loss_mlflow(e, train_loss_)
...
if __name__ == "__main__":
    with open("../configs/config.json", "rb") as f:
        configs = json.load(f)
    trainer = LightGCNTrainer(**configs)
    trainer.run()
