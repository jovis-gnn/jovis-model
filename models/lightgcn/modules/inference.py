...
from models.lightgcn.modules.dataset import LightGCNData, LightGCNTest
from models.lightgcn.utils.helper import get_filtered_dataset_from_s3


class LightGCNInferencer:
    def __init__(
        self,
    ...
    def load_from_s3(self):
        ...
        objects = bucket.objects.filter(
            Prefix=f"{self.company_id}/lightgcn/{self.etl_id}/{self.model_id}"
        )
        obj_list = list(objects.all())

        for obj in obj_list:
            stream_ = io.BytesIO(obj.get()["Body"].read())
            if "lightgcn_start.pth" in obj.key:
                self.checkpoint = torch.load(stream_)
                self.torch_model = LightGCN(
    ...
    def get_dataset(self, test_batch_size=1):
        ...
        self.test_ds = LightGCNTest(self.data_dict)
