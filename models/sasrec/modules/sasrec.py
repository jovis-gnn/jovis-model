...
class SASRec(nn.Module):
    def __init__(self, config, num_user, num_item, device):
        ...
        layer_basic_args = (self.embed_size, self.num_heads, self.dropout_ratio, self.attn_dropout_ratio)
        self.layers = nn.ModuleList([Basic_Block(*layer_basic_args, config["pre_ln"], layer_idx == (self.num_layers - 1)) for layer_idx in range(self.num_layers)])
    ...
    def get_history_embedding(self, history, history_mask):
        ...
        for init_layer in self.init_layers:
            history_embed = init_layer(history_embed)
    ...
    def _get_unif_reg(self, history_embed, pos_embed, neg_embed, ran_embed):
        item_args = (history_embed, pos_embed, ran_embed) + self.link_loss * (neg_embed,)
        unif_reg = self.beta * sum(list(map(lambda x: torch.pdist(x, p=2).pow(2).mul(-2.).exp().sum(), item_args)))

        unif_reg += self.beta * torch.cdist(history_embed, ran_embed, p=2).pow(2).mul(-2.).exp().sum()

        return unif_reg

    def _get_norm_reg(self, history_embed, pos_embed, neg_embed, ran_embed):
        raw_norm_reg = history_embed.norm(dim=-1).pow(2).sum() + pos_embed.norm(dim=-1).pow(2).sum()

        num_neg, num_ran = list(map(lambda arr: 1 if len(arr.size()) == 2 else arr.shape[1], [neg_embed, ran_embed]))

        raw_norm_reg += ran_embed.norm(dim=-1).pow(2).sum() * 1 / num_ran

        if self.link_loss:
            raw_norm_reg += neg_embed.norm(dim=1).pow(2).sum() * 1 / num_neg
